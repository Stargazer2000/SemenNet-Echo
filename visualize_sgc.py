...
class SegGradCAMplot(SegGradCAM):
    ...
    def explainBase(self, title1, title1bias, start_save_name, pixel=False):
        ...
        # plt.axis('off')
        self.ximg=self.ximg.squeeze(0).permute(1,2,0).cpu()
        print('ximg',type(self.ximg))
        ...
        if pixel:
            ...
            print('roi_contour1:',roi_contour1)
        ...
        plt.imshow(self.cam, cmap='jet',  # vmin=0,vmax=1,
                   alpha=0.6)
        # pylab.show()
        plt.colorbar(fraction=0.046, pad=0.04, ticks=[0, 0.2, 0.4, 0.6, 0.8, 1])
        plt.show()
